$(function() {
	...
	$(".codes-container").on('mouseover', '.start-code', function(e) {
...
function replace_incorrect_characters($field)
{
	...
	for (var i in mappings) {
		code = code.replace(i, mappings[i]);
	}
	$field.val(code);
}
...
function validate_field($field) 
{
	...
	var request = $.getJSON('codes/validate/' + code + '/' + user_id, function(data) {
		if (data.valid) {
			...
			$control_group.addClass('warning');
			$control_group.removeClass('success');
			var msg = data.reason;
			msg = msg + ' - try our <a href="codes/finder">Near Miss Finder</a>';
			$row.find('.help').html(msg);
		}
